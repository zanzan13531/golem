...
def golemify(B, x): 
    ...
    for i in range (d): # outer sum
        for k in range(n): # inner sum
            ...
            sum = sum + innerThing # adds the inner stuff to them sum

    logSum = math.log(sum) # log(sum stuff) : log of the sum stuff

    firstHalf = logSum * d / 2.0 # d*log(sum stuff)/2 : first half of the equation, pretty sure if this is a scalar
